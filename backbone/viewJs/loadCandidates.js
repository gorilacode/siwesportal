$(document).ready(function(){
	...
	function image_loaded() {
		...
		function get_table_data(){
			var referralCode = $("#getcode").val();

			$.ajax({
				url: '../../backbone/controller/getCandidates.php',
				type: 'POST',
				...
				if (data.data != 'zero') {
					...
					$("#nofill").html('<h5 class="text-center"><i> YOU ARE SEEING THIS MESSAGE BECAUSE YOU HAVE NOT ADDED YOUR REFFERAL CODE ON YOUR PROFILE OR A STUDENT HAS NOT YET BEEN ASSIGNED TO YOU </i> </h5>');
				}
		...
		function manage_data(data){
				...
				$.each(data, function(key, value){
					...
					row += '<div class="btn btn-primary"><a sytle="background-color: white; " href="activities.php?tokenId='+ value.tokenId +'&amp;referralCode='+value.referralCode+'"> <i class="fa fa-eye"> </i> </a> </div>';
					row += '</td>';
					row += '</tr>'; 
				});
