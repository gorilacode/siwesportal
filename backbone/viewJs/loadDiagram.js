$(document).ready(function(){
	...
	function image_loaded() {
		...
		function get_table_data(){
			...
			$.ajax({
				url: '../backbone/controller/getPersonalLogbook.php',
				type: 'POST',
		...
		function manage_data(data){
				...
				$.each(data, function(key, value){
					...
					if(value.status == 'seen'){
						row += '<td class="text-center" ><span class="badge badge-success"> '+ value.status +' </span></td> '; 
					}else if(value.status == 'unseen'){
						row += '<td class="text-center" ><span class="badge badge-warning"> '+ value.status +' </span> </td>';
					}

					if(value.remark == 'satisfied'){
						row += '<td class="text-center upper" hidden> <i class="fa fa-thumbs-up  ">'+ value.remark +' </td>';
					}else if(value.remark == 'not satisfied'){
						row += '<td class="text-center upper" hidden> <i class="fa fa-thumbs-down ">'+ value.remark +' </td>';
					}else if(value.remark == '--'){
						row += '<td class="text-center upper" hidden> <i class="fa fa-spinner ">'+ value.remark +' </td>';
					}
					row += '<td class="text-center upper" hidden>'+ value.comment +'</td>';
					...
					row += '<button class=" btn btn-primary btn-sm log-update" data-toggle="modal" data-target="#log-Updates"> <i class="fa fa-edit"></i> </button>'; 
					row += '</td>';
					row += '</tr>'; 
				});
